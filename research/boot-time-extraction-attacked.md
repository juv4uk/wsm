...
**Оновлення, 2026-09-03 — тепер порівняно з реальним фізичним виміром,
не лише застереженням.** Власник реально запустив
`physical-boot-probe.c` на своєму i5-6400 через Ventoy. Реальний
результат: `2712 MHz-еквівалент` — стабільний, один вимір, впритул
збігається з паспортною базовою частотою i5-6400 (2.70 ГГц), тоді як
TCG-числа плавали між прогонами цієї самої сесії (2699, 2712, 2718,
2723 МГц — різні запуски, той самий віртуальний "кремній"). Це саме
підтверджує застереження вище: TCG-число не було підтвердженням, воно
випадково опинилось близько; реальний вимір — окремий, і саме він тепер
`LIVE-CONFIRMED` для фізичного заліза.
