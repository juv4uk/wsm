...
- `readable != physically representative`. Успішне читання з
  ...
  на інші без окремої перевірки там теж. **2026-09-03: третя
  реальність більше не теоретична** — `wsm-os/probe/physical-boot-
  probe.c` реально запущено на фізичному i5-6400 власника; жива
  ревізія мікрокоду на реальному залізі — `0xC2`, точно та сама, що
  `STATIC-CONFIRMED` усередині самої прошивки F22e — перше пряме
  підтвердження, що ці дві реальності справді збігаються в цій одній
  конкретній точці (див. `wsm-os/probe/README.md`).
